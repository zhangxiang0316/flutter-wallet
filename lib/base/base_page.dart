  ...
  Widget build(BuildContext context) {
    ...
    return LifecycleWrapper(
      onLifecycleEvent: (LifecycleEvent lifecycleEvent) {
        ...
        final pageController = Get.find<T>();
        switch (lifecycleEvent) {
          case LifecycleEvent.visible:
            // 页面变为可见
            pageController.onPageVisible();
            break;
          case LifecycleEvent.inactive:
            // 页面进入非活动状态
            pageController.onPageInActive();
            break;
          case LifecycleEvent.active:
            // 页面进入活动状态
            pageController.onPageActive();
            break;
          case LifecycleEvent.invisible:
            // 页面变为不可见
            pageController.onPageInVisible();
            break;
          default:
        }
      },
      child: GetBuilder<T>(
        builder: (controller) => buildWidget(context, controller),
        assignId: true,
      ),
    );
  }
