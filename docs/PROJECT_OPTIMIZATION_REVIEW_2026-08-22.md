...
### 5.2 自动化测试与 CI（自动化测试基线已完成，2026-08-23）

已建立自动化测试与本地质量检查基线；当前项目不引入远程 CI 工作流。

测试覆盖：

1. [x] TRON 节点篡改交易内容的对抗测试。
2. [x] EVM Gas、pending nonce、EIP-1559 和 signer 地址测试。
3. [x] Token 余额和原生手续费余额联合校验。
4. [x] 全链二维码和链不匹配测试。
5. [x] 余额缓存链配置重新绑定、过期状态和部分失败保留测试。
6. [x] 首页并发刷新、切换钱包和旧请求覆盖测试；刷新开始标记提前到缓存读取之前，避免缓存读取窗口重复发起网络请求。
7. [x] 同交易多 Token 事件合并测试。
8. [x] 私钥显示、剪贴板条件清理和应用生命周期竞态测试。
9. [x] 地址、金额、付款 URI 增加固定随机种子的属性测试和畸形输入 fuzz 测试，覆盖全部内置链。
10. [ ] 密钥写入、迁移和删除的部分失败测试与 4.6 仓储原子性改造一起完成，避免先把当前非原子行为固化成错误断言。

本地质量检查：

1. [x] 对 `lib/`、`test/` 下的手写 Dart 文件执行格式检查；`lib/generated/` 由对应生成器校验，避免生成器与 SDK formatter 版本差异造成假失败。
2. [x] 重新生成本地化文件并通过 `git diff --exit-code` 检查生成文件是否同步。
3. [x] 执行 `flutter analyze --no-fatal-infos` 和完整 `flutter test`；现有 14 条 info 留给 5.3 清零。
4. [x] 执行 `flutter build apk --debug` 作为 Android 构建冒烟测试。
5. [x] 新增 `scripts/check_secrets.sh`，检查被跟踪的敏感配置文件、常见云平台 Token、PEM 私钥和生产代码中的硬编码钱包私钥/助记词；扫描输出只报告文件名，不回显疑似秘密。
6. [x] Golden 测试移除本机绝对路径和 macOS 专属字体，改为读取当前 Flutter SDK 自带字体。
7. [ ] 暂不创建 `.github/workflows/` 远程自动化流程；需要时再将上述本地命令接入 CI。

#### 验收标准

- 本地质量检查命令全部通过，不依赖 `.env.local` 或真实 API Key；
- 格式、本地化生成文件、静态分析、测试、敏感信息和 Debug APK 构建均可独立执行；
- 首页缓存读取期间重复刷新只产生一个网络请求，切换钱包后旧请求不能覆盖新钱包余额；
- 属性测试使用固定随机种子，失败可以稳定复现；
- Golden 测试不再依赖开发者用户名、macOS 系统字体或固定 Flutter 安装目录。
