...
### 4.5 日志可能暴露 API Key、地址和余额信息（已完成，2026-08-23）
...
- 新增 `SafeLog`，统一脱敏 URL query 中的 API Key、token、password、secret 等字段；
- EVM 地址和私钥样式字符串在日志中只保留安全摘要或直接替换；
- RPC、余额刷新、交易历史和行情错误统一通过脱敏日志入口记录；
- 余额调试日志移除具体金额、地址和合约地址，仅保留币种、精度和成功/失败状态；
- 增加 `safe_log_test.dart`，覆盖凭据、地址和私钥样式数据的脱敏。
